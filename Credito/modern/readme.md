...
- **Validar anular movimiento caja (JWT):** `GET /api/v1/credito/validar-anular-movimiento-caja?oficinaId=&movimientoCajaId=` — paridad **`ValidarAnularMovimientoCaja`** + UI CajaDiario; `{ bloqueadoPorPagosCuota }` (`true` = INI con cuotas PAG → no anular).
- **Anular movimiento caja (JWT, escritura):** `POST /api/v1/credito/anular-movimiento-caja` — **`usp_MovimientoCaja_Del`**; body `{ oficinaId, movimientoCajaId, observacion }` (observación obligatoria); `usuarioId` desde JWT; **409** si ya anulado, caja cerrada o INI con cuotas PAG.

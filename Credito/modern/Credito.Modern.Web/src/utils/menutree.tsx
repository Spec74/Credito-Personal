...
function toMenuNode(item: MenuItemDto): AntMenuItem {
  ...
  const text = formatMenuLabel(item.denominacion) || `Menú ${item.menuId}`
  return {
    key,
    icon: menuItemIcon(item.icono, item.denominacion, item.modulo),
    label: <span className="credix-menu-label">{text}</span>,
  }
}
...
export function buildAntMenuItems(items: MenuItemDto[]): AntMenuItem[] {
  ...
  for (const parent of parents) {
    ...
    const parentLabel =
      formatMenuLabel(parent.denominacion) || `Grupo ${parent.menuId}`
    submenuParents.push({
      key: `parent-${parent.menuId}`,
      icon: parentMenuIcon(parent.denominacion),
      label: parentLabel,

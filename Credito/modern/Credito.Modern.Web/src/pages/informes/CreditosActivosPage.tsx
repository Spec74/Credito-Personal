...
export function CreditosActivosPage() {
  ...
  const consulta = useMutation({
    mutationFn: (v: FormValues) =>
      fetchCreditosActivos(toParams(v, session?.usuarioId)),
  })

  const csv = useMutation({
    mutationFn: (v: FormValues) =>
      downloadCreditosActivosCsv(toParams(v, session?.usuarioId)),
  })

  const pdf = useMutation({
    mutationFn: (v: FormValues) =>
      downloadCreditosActivosPdf(toParams(v, session?.usuarioId)),
  })
  ...
  return (
    <CredixInformePage
      ...
      subtitle="Créditos activos por oficina y rango de fechas; opción de filtrar solo su gestión."

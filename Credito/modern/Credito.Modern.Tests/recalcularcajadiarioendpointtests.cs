...
public class RecalcularCajaDiarioEndpointTests : IClassFixture<CreditoModernWebApplicationFactory>
{
    private readonly HttpClient _client;

    public RecalcularCajaDiarioEndpointTests(CreditoModernWebApplicationFactory factory)
    {
        _client = factory.CreateClient();
    }

    [Fact]
    public async Task Recalcular_caja_diario_sin_jwt_devuelve_401()
    {
    ...
    [Fact]
    public async Task Recalcular_caja_diario_oficina_distinta_al_token_devuelve_403()
    {
    ...
    [Fact]
    public async Task Recalcular_caja_diario_parametros_alineados_no_es_401()
    {
        if (string.Equals(Environment.GetEnvironmentVariable("CI"), "true", StringComparison.OrdinalIgnoreCase))
        {
            return;

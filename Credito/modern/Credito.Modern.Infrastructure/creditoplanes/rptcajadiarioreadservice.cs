...
public sealed class RptCajaDiarioReadService(IOptions<SqlDatabaseOptions> options)
    : IRptCajaDiarioReadService
{
    private readonly string _connectionString = options.Value.ConnectionString;

    public async Task<IReadOnlyList<RptCajaDiarioRowDto>> ListarAsync(
        DateTime fechaIni,
        DateTime fechaFin,
        int? usuarioId,
        int oficinaId,
        CancellationToken cancellationToken = default)
    {
        ...
        var rows = await connection.QueryAsync<RptCajaDiarioRowDto>(command).ConfigureAwait(false);
        return rows.ToList();
    }
}

...
public sealed class RptCreditosCierresReadService(IOptions<SqlDatabaseOptions> options)
    : IRptCreditosCierresReadService
{
    private readonly string _connectionString = options.Value.ConnectionString;

    public async Task<IReadOnlyList<RptCreditosCierresRowDto>> ListarAsync(
        DateTime fechaIni,
        DateTime fechaFin,
        int? usuarioId,
        int oficinaId,
        CancellationToken cancellationToken = default)
    {
        ...
        var command = new CommandDefinition(
            "CREDITO.usp_RptCreditosCierres",
            new
            {
                FechaIni = fechaIni,
                FechaFin = fechaFin,
        ...
        var rows = await connection.QueryAsync<RptCreditosCierresRowDto>(command).ConfigureAwait(false);
        return rows.ToList();
    }
}

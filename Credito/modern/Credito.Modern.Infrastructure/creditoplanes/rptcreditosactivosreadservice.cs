...
public sealed class RptCreditosActivosReadService(IOptions<SqlDatabaseOptions> options)
    : IRptCreditosActivosReadService
{
    private readonly string _connectionString = options.Value.ConnectionString;

    public async Task<IReadOnlyList<RptCreditosActivosRowDto>> ListarAsync(
        DateTime fechaIni,
        DateTime fechaFin,
        int? usuarioId,
        int oficinaId,
        CancellationToken cancellationToken = default)
    {
        ...
        var command = new CommandDefinition(
            "CREDITO.usp_RptCreditosActivos",
            new
            {
                FechaIni = fechaIni,
                FechaFin = fechaFin,
        ...
        var rows = await connection.QueryAsync<RptCreditosActivosRowDto>(command).ConfigureAwait(false);
        return rows.ToList();
    }
}
